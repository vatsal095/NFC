...
import "contracts-upgradeable/presets/ERC721PresetMinterPauserAutoIdUpgradeable.sol";

contract NFC is ERC721PresetMinterPauserAutoIdUpgradeable {
    mapping(uint256 => address) private _originalOwner;
    string private _contractURI;

    function init() public initializer {
        __ERC721PresetMinterPauserAutoId_init(
            "Spectral Credit Tokens",
            "NFC",
            "http://165.232.180.65/api/submit/"
        );
        _contractURI = "http://165.232.180.65/api/submit/";
    }
    ...
    function changeContractURI(string memory newContractURI) public {
        require(
            ...
            "ERC721: must have admin role to change"
        );
